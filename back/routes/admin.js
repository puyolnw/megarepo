...
const { User, SystemSetting, SerialHistory, Activity, Serial, Checkin, ActivityReview, AcademicYear } = require('../models');
...
router.get('/academic-years', requireRole('ADMIN'), async (req, res) => {
  try {
...
router.post('/academic-years', requireRole('ADMIN'), async (req, res) => {
  try {
    const { year, year_thai, start_date, end_date, is_active = true } = req.body;
    
    // Validate required fields
    if (!year || !year_thai || !start_date || !end_date) {
      return res.status(400).json({
        success: false,
        message: 'Year, year_thai, start_date, and end_date are required'
      });
    }
    
    // Check if year already exists
    const existingYear = await AcademicYear.findOne({ where: { year } });
    if (existingYear) {
      return res.status(400).json({
        success: false,
...
router.put('/academic-years/:id', requireRole('ADMIN'), async (req, res) => {
  try {
    ...
    const { year, year_thai, start_date, end_date, is_active } = req.body;
    
    await academicYear.update({
      year: year || academicYear.year,
      year_thai: year_thai || academicYear.year_thai,
...
router.delete('/academic-years/:id', requireRole('ADMIN'), async (req, res) => {
  try {
    const academicYear = await AcademicYear.findByPk(req.params.id);
    ...
    // Check if there are activities using this academic year
    const activitiesCount = await Activity.count({ where: { academic_year: academicYear.year } });
    if (activitiesCount > 0) {
      return res.status(400).json({
        success: false,
        message: `Cannot delete academic year. There are ${activitiesCount} activities using this year.`
      });
    }
...
router.get('/activities', requireRole('ADMIN', 'STAFF'), async (req, res) => {
  try {
    // Get activities with creator info and academic year
    ...
    const activitiesWithCount = await Promise.all(
      activities.map(async (activity) => {
        ...
        return {
          ...
          academic_year: activity.academic_year || activity.academicYear?.year || null,
          academic_year_thai: activity.academicYear?.year_thai || null,
