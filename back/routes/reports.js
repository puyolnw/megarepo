...
router.get('/members', requireRole('ADMIN'), async (req, res) => {
  try {
    const { startDate, endDate, program, enrollmentYear, academicYear } = req.query;
    ...
    // Add academic year filter to activities
    const activityWhere = {};
    if (academicYear) {
      activityWhere.academic_year = academicYear;
    }
    ...
    const academicYearProgress = await SerialHistory.findAll({
      attributes: [
        [col('activity.academic_year'), 'academic_year'],
        [fn('SUM', col('hours_earned')), 'totalHours']
      ],
      include: [{
        model: User,
      ...
      order: [[col('activity.academic_year'), 'DESC']]
    });

    // Get academic year activities count
    const academicYearActivities = await Activity.findAll({
      attributes: [
        'academic_year',
    ...
    res.json({
      ...
      data: {
        ...
        academicYearActivities: academicYearActivities.map(item => ({
          academic_year: item.academic_year,
          activityCount: item.dataValues.activityCount
        })),
        topStudents: topStudents.map(student => ({
          user_id: student.user_id,
          total_hours: parseInt(student.dataValues.total_hours) || 0,
          activity_count: parseInt(student.dataValues.activity_count) || 0,
          user: {
            id: student.user.id,
...
router.get('/activities', requireRole('ADMIN'), async (req, res) => {
  try {
    const { startDate, endDate, status, category, academicYear } = req.query;
    
    // Build where conditions
    const activityWhere = getActivityDateRange(startDate, endDate);
    if (status) activityWhere.status = status;
    if (academicYear) activityWhere.academic_year = academicYear;
    ...
    const topActivities = await sequelize.query(`
      ...
      LEFT JOIN activity_reviews ar ON a.id = ar.activity_id
      ${activityWhereClause}
      GROUP BY a.id, a.title, a.start_date, a.status, a.academic_year
      ORDER BY participantCount DESC
      LIMIT 10
    `, {
      type: QueryTypes.SELECT,
...
router.get('/evaluations', requireRole('ADMIN'), async (req, res) => {
  try {
    const { startDate, endDate, activityId, ratingRange, academicYear } = req.query;
    ...
    const averageRatings = await ActivityReview.findOne({
      ...
      attributes: [
        ...
        [fn('AVG', col('organization_rating')), 'organization'],
        [fn('AVG', col('venue_rating')), 'venue']
      ]
    });
    ...
    const topRatedActivities = await sequelize.query(`
      ...
      ${whereClause}
      GROUP BY a.id, a.title, a.start_date, a.academic_year
    ...
    res.json({
      ...
      data: {
        summary: {
          ...
          averageOrganizationRating: parseFloat(averageRatings?.dataValues?.organization) || 0,
          averageVenueRating: parseFloat(averageRatings?.dataValues?.venue) || 0
        },
        ratingTrend,
        ratingDistribution: [1,2,3,4,5].map(rating => {
          const found = ratingDistribution.find(r => r.overall_rating === rating);
